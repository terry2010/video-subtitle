...
python app.py translate --srt-file test_jpn.mp3.ai.ja.srt --src-lang ja --tgt-lang zho_Hans --device cuda --compute-type int8

直接从音频文件生成翻译后的字幕:

python app.py translate_from_audio --audio-path test.mkv --timeout 120 --subtitle eng --audio jpn --audio-format wav --audio-sample-rate 16000 --model-size large_v2 --device cuda --compute-type int8 --src-lang eng --tgt-lang zho_Hans --print-info
```
